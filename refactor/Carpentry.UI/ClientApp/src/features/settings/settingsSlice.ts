...
export const loadTrackedSets = createAsyncThunk<NormalizedList<TrackedSetDto>, boolean>(
  'trackedSets/loadTrackedSets',
  async (showUntracked) => trackedSetsApi.getTrackedSets(showUntracked)
);
...
export const modifyTrackedSets = createAsyncThunk<void, {action: TrackedSetsApiAction, setId: number, showUntracked: boolean}>(
  ...
  async(props, thunkApi) => {
    ...
    thunkApi.dispatch(loadTrackedSets(props.showUntracked))
  }
);
...
export const settingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(loadTrackedSets.pending, (state, action) => {
      ...
      state.showUntrackedSets = action.meta.arg;
    });
