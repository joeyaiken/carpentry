...
function searchCardSearchInventory(dispatch: Dispatch, state: AppState, card: MagicCard): any{
    ...
    const searchInProgress: boolean = state.data.isLoading.cardSearchInventoryDetail;
    if(searchInProgress){
    ...
    api_Inventory_GetByName(card.name).then((results) =>{
...
function searchCards(dispatch: Dispatch, state: AppState): any{
    ...
    const searchInProgress: boolean = state.data.isLoading.cardSearchResults; //state.cardSearch.isLoading;
    ...
    if(cardSearchMethod === "web"){
        api_Cards_SearchWeb(state.ui.cardSearchFilterProps.cardName, state.ui.cardSearchFilterProps.exclusiveName).then((results) =>{
            dispatch(apiDataReceived(_localApiScope, results));
        });
        ...
        api_Cards_SearchInventory(param).then((results) => {
            dispatch(apiDataReceived(_localApiScope, results));
        });

    } else {
        api_Cards_SearchSet(state.ui.cardSearchFilterProps).then((results) => {
            dispatch(apiDataReceived(_localApiScope, results));
        })
    }
}
...
function addDeckCard(dispatch: Dispatch, state: AppState, inventoryCard: InventoryCard): any{
    ...
    //console.log
    api_Decks_AddCard(deckCardDto).then(() => {
...
function moveDeckCard(dispatch: Dispatch, state: AppState, card: InventoryCard, deckCardId: number): any{
    ...
    //maybe build a DTO
    api_Decks_UpdateCard(deckCardDto).then(() => {
