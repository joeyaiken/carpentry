...
namespace CarpentryMigrationTool
{
    public class DataMigrationService //: IDataMigrationService
    {
        ...
        private void SaveDb()
        {
            ...
            var itemExports = _cardContext.InventoryCards.Select(x => new InventoryCardBackup
            {
                Id = x.Id,
                MultiverseId= x.MultiverseId,
                //IsFoil = x.IsFoil,
            ...
            //query decks
            var deckExports = _cardContext.Decks.Select(x => new DeckBackup
            {
                    Id = x.Id,
                    Name = x.Name,
            ...
            //query deck cards
            var deckCardsExport = _cardContext.DeckCards.Select(x => new DeckCardBackup
            {
                DeckId = x.DeckId,
                InventoryCardId = x.InventoryCardId
            }).OrderBy(x => x.DeckId).ToList();

            //query set codes
            var setCodes = _cardContext.Sets.Select(x => x.Code).OrderBy(x => x).ToList();

            var deckObj = JObject.FromObject(new DeckTextBackupDto
            {
                Decks = deckExports,
                DeckCards = deckCardsExport
            });

            var invObj = JObject.FromObject(new InventoryTextBackupDto
            {
                SetCodes = setCodes,
                Cards = itemExports
