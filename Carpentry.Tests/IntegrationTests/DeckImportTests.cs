...
";
            ...
            var importPayload = new CardImportDto()
            {
                ImportType = CardImportPayloadType.Carpentry,
                ImportPayload = payloadString,
            };
            ...
            var response = await client.PostAsync(apiEndpoint, queryParamStringContent);

            var responseContent = await response.Content.ReadAsStringAsync();

            var searchResult = JsonConvert.DeserializeObject<ValidatedDeckImportDto>(responseContent);
